...
def file_logger(fname):
    def logger(f):
        def attribute_writer(*args, **kwargs):
            with open(fname, 'a') as fout:
                fout.write(f'{datetime.now()} {f.__name__}\n')
                if len(args) > 0:
                    fout.write('positional arguments: \n')
                    for i in args:
                        fout.write(f'\t{i}\n')
                if len(kwargs) > 0:
                    fout.write('named arguments: \n')
                    for i in kwargs:
                        fout.write(f'\t{i}: {kwargs[i]}\n')
                
                result = f(*args, **kwargs)
                fout.write(f'returned {result}')
                return result
        return attribute_writer
    return logger
